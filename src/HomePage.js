...
function HomePage() {
    ...
    useEffect(() => {
        setActiveSection('hero-section'); // Set default active section to 'hero-section'
            ...
            <div className='HomePage '>
                <nav className="navbar navbar-expand-lg bg-light fixed-top">
                    <div className="container">
                        <Link className="navbar-brand" to="/">Aurassance.com</Link>
                        <button className="navbar-toggler" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasNavbar" aria-controls="offcanvasNavbar">
                            <span className="navbar-toggler-icon"></span>
                        </button>
                        <div className="offcanvas offcanvas-end" tabIndex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
                            <div className="offcanvas-header">
                                <h5 className="offcanvas-title" id="offcanvasNavbarLabel">Aurassance.com</h5>
                                <button type="button" className="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                            </div>
                            <div className="offcanvas-body">
                                <ul className="navbar-nav justify-content-end flex-grow-1 pe-3">
                                    <li className={`nav-item ${activeSection === 'hero-section' ? 'active' : ''}`}>
                                        <button className="nav-link" onClick={() => scrollToSection('hero-section')}>Home</button>
                                    </li>
                                    <li className={`nav-item ${activeSection === 'services-section' ? 'active' : ''}`}>
                                        <button className="nav-link" onClick={() => scrollToSection('services-section')}>Services</button>
                                    </li>
                                    ...
                                    <li className={`nav-item ${activeSection === 'contact-section' ? 'active' : ''}`}>
                                        <button className="nav-link" onClick={() => scrollToSection('contact-section')}>Contact</button>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div id="hero-section" className='hero-section'>
                    ...
                    <div className='centered-text'>
                        <h1>Unlock Your Brands Potential: Elevate With Custom Websites and Targeted Ads.</h1>
                        <button onClick={() => scrollToSection('services-section')}>Get started</button>
                    </div>
                </div>
            </div>
